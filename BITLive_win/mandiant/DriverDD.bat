...
echo      -driver   optional name of driver to acquire
...
:scriptname
echo      ^<module name=^"w32driver-memoryacquire^" version=^"1.3.22.2^" /^> >>"%OUTPATH%"
...
if not "%DRIVER_NAME%" == "" echo ^<param name=^"driver name^"^>^<value xsi:type=^"xsd:string^"^>%DRIVER_NAME%^</value^>^</param^> >>"%OUTPATH%" 
